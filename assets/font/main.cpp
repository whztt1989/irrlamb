		...
		else if (event.EventType == EET_GUI_EVENT)
		{
			...
			case EGET_BUTTON_CLICKED:
				...
				if (id == MYGUI_CREATE)
				{
					...
					core::stringw FontName = cbo->getText();
					FontName.make_lower();

					cbo = (IGUIComboBox*)env->getRootGUIElement()->getElementFromId(MYGUI_SIZE,true);
					int fontsize = cbo->getSelected();

					core::stringw FontSizeName = cbo->getText();
					FontSizeName = FontSizeName.subString(0, FontSizeName.size()-2);

					edt->setText(core::stringw(FontName + core::stringw("_") + FontSizeName).c_str());
...
void createGUI(IrrlichtDevice* device, CFontTool* fc)
{
	...
	env->getSkin()->setColor(gui::EGDC_WINDOW, video::SColor(255, 0, 0, 0));
	...
	*/
	...
	// font image
	gui::IGUIImage *img = env->addImage(0, core::position2d<s32>(0,0), false,0, MYGUI_IMAGE);
	img->setRelativePosition(core::rect<s32>(0,20,1280,720));

	// font scrollbar
	IGUIScrollBar *scrl= env->addScrollBar(true,core::rect<s32>(0,0,1280,20),0, MYGUI_CURRENTIMAGE);
	scrl->setMax(0);
	scrl->setSmallStep(1);
	...
	win->setRelativePosition( core::rect<s32>(10+700,10+50,200+700,yp+50));
}

int main()
{
	IrrlichtDevice* device =createDevice(video::EDT_OPENGL, core::dimension2du(1280, 720));
